...
>(({ className, children, onPointerDownOutside, onInteractOutside, ...props }, ref) => {
  // Handler to prevent closing when clicking on Google Places autocomplete dropdown
  const handlePointerDownOutside = (event: React.PointerEvent) => {
    const target = event.target as HTMLElement;
    // Check if click is on Google Places autocomplete elements
  ...
  const handleInteractOutside = (event: Event) => {
    const target = event.target as HTMLElement;
    // Check if interaction is on Google Places autocomplete elements
    if (target.closest('.pac-container') || target.classList.contains('pac-item') || target.closest('.pac-item')) {
      event.preventDefault();
      return;
    }
